...
def build_worker_image():
    print("🛠️ Checking if 'vgpu-worker' Docker image exists...")
...
def sync_docker_time():
    ...
    try:
        ...
        print(f"⚠️ Warning: Could not sync clock: {e}")
...
def main():
    ...
    if not check_and_start_docker():
        print("⚠️ Proceeding without Docker (falling back to simulation mode)...")
    else:
        build_worker_image()
        sync_docker_time()
